...
- float 를 부여한 요소는 normal flow가 아니게 되며, float를 부여하지 않은 text 나, inline 요소가 float 를 부여한 요소의 좌측 또는 우측을 따라 흐르게 배치된다.<br>-> 즉, float 를 부여한 요소는 그 요소의 크기만큼 영역을 차지하고, float 를 부여한 그 요소의, 형제 요소들은 float 를 부여한 요소만큼의 영역에 침범을 하지 못하고 옆에서 흐르게 된다. 
