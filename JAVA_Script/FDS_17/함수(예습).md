...
- 재귀함수는 자기 자신을 무한 호출하기 때문에 탈출 조건을 반드시 만들어야 한다. 
...
- 함수의 변하지 않는 공통된 코드는 함수 표현식을 통해 미리 따로 정의해두고 (= 콜백함수), 변경되는 일은 <u>콜백함수를 매개변수를 통해 전달받는 함수를 고차함수</u>라고 한다. <br>즉, <u>**매개변수를 통해 전달되는 함수**</u>를 콜백함수라고 한다.
...
- 어떠한 외부 상태라도, 해당 외부 상태에 의존하지 않고, **외부 값(전역변수의 값)을 변경시키지도 않은 함수**
...
- 👍 **<u>오직 매개변수를 통해 함수 내부로 전달된 인수에게만 의존하는 함수</u>**
  ...
  console.log(count) // 1     │
  //							│ 순수 함수가 반환한 결과값을, 변수에 재할당하여 외부 상태를 변경
  count = increase(count)//   │
  console.log(count) // 2 <-──┘
  ```
